	...
	if(!strcmp(cmd,"verify")){
		...
		keyname = argv[2];
                ...
                if(5 == argc) {
                        ...
                        const char* src_sha256 = argv[3];
                        len = strlen(src_sha256) / 2;
                        int i = 0; 

                        if(32 != len){
                                printf("sha256 len must be 64, but %d!\n", len);
                                return -1;
                        }
                        char tmp[4];
                        tmp[2] = '\0';
                        for(i = 0; i < len; ++i)
                        {
                                tmp[0] = src_sha256[i * 2 + 0];
                                tmp[1] = src_sha256[i * 2 + 1];
                                sha256Buf[i] = simple_strtoul(tmp, NULL, 16);
                        }
                        addr = (unsigned)sha256Buf;
                }
		int query_status = -1;
